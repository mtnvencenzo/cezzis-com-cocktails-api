...
public class Auth0ManagementTokenService(IOptions<Auth0Config> auth0Config) : IAuth0ManagementTokenService
{
    ...
    private async Task<string> GetManagementApiTokenInternalAsync()
    {
        ...
        try
        {
            var data = $"grant_type=client_credentials&client_id={this.auth0Config.Value.ClientId}&client_secret={this.auth0Config.Value.ClientSecret}&audience={Uri.EscapeDataString(this.auth0Config.Value.Domain)}%2Fapi%2Fv2%2F";

            var content = new StringContent(data, Encoding.UTF8, "application/x-www-form-urlencoded");
            var response = await client.PostAsync($"{this.auth0Config.Value.Domain}/oauth/token", content);
