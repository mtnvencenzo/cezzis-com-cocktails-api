...
{
    public ManageFavoriteCocktails_Tests() { }

    [Fact]
    public async Task managefavorites__returns_matching_account()
    {
        // Arrange
        var faker = new Faker();
        ...
        var request = new ManageFavoriteCocktailsRq
        (
            CocktailActions:
            [
                new CocktailFavoriteActionModel
                (
                    CocktailId: account.FavoriteCocktails.First(),
                    Action: CocktailFavoritingActionModel.Remove
                ),
                new CocktailFavoriteActionModel
                (
                    CocktailId: "paloma",
                    Action: CocktailFavoritingActionModel.Add
                )
            ]
        );
        ...
        // act
        var response = await AccountsApi.ManageFavoriteCocktails(request, services);
    ...
    private Account GetUpdatedAccount(Account account, ManageFavoriteCocktailsRq request)
    {
